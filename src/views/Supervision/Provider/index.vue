...
import { getProviders, providerStates, updateState, removeProvider } from '@/api/supervision/provider';
import { Provider, ProviderSchema, ProviderState } from '@/api/supervision/provider/types';
...
// 创建表格hook和筛选条件
const { list, fetchList, pagination, isLoading } = useFetchList(getProviders);
const schema: Ref<ProviderSchema> = ref({});
 ...
 * 表格筛选
 */
const onSearch = (requestSchema: ProviderSchema) => {
...
const onRemove = async (row: Provider) => {
    ...
    const [error, result] = await pw(removeProvider(row.id));
 ...
 * 是否已启用
 */
const isOpenState = (state: ProviderState): boolean => {
    return state == '1';
};
 ...
 * 切换状态
 */
const onChangeState = (row: Provider) => {
    row.loading = true;

    const afterState: ProviderState = row.state == '1' ? '0' : '1';
            ...
            <template #filters>
                ...
                <ElFormItem>
                    <ElSelect v-model="schema.state" placeholder="请选择状态">
                        ...
                        <ElOption v-for="provider in providerStates" :key="provider.value" :label="provider.label" :value="provider.value" />
                    </ElSelect>
                </ElFormItem>
                <ElFormItem>
